...
var JSONDB = (function () {
    ...
    JSONDB.prototype.createServer = function (name, username, password, connect) {
        ...
        var path = Util.getServersPath() + name;
        if (null !== path && username !== null) {
            ...
            require('jdb-configuration').addUser(name, username, password);
    ...
    JSONDB.prototype.connect = function (server, username, password, database) {
        return require('jdb-database')(server, username, password, database);
    };
    ...
    JSONDB.prototype.async.connect = function (server, username, password, database, callback) {
        ...
        setImmediate(function () {
            try {
                callback(null, require('jdb-database')(server, username, password, database));
            } catch (e) {
                callback(e, null);
            }
        }.bind(this));
    ...
    JSONDB.quote = function (value) {
        return require('jdb-queryparser').quote(value);
    };

    /**
     * Gets all servers managed by an user
     * @param {string} username
     * @param {string} password
     * @return {Array}
     */
    JSONDB.getServerList = function (username, password) {
        var _f = require('fs');
        var Util = require('jdb-util');

        return _f.readdirSync(Util.getServersPath());
    };

    /**
     * Connects an user through the web administration
     * @param {string} username
     * @param {string} password
     * @return {object}
     */
    JSONDB.connectUserByWebAdmin = function(username, password) {
        username = username || null;

        if (null === username) {
            return {success: false, error: ['An error occured, please try again. If the error persist, contact your administrator. Error Code: 001']};
        }

        var _c = require('jdb-configuration');
        var _u = require('jdb-util');
        var _p = require('path');

        var config = _c.getConfig('users');

        var username_hash = _u.crypt(username);
        var password_hash = _u.crypt(password);

        for (var server in config) {
            if (config.hasOwnProperty(server)) {
                if (config[server].username === username_hash && config[server].password === password_hash) {
                    return {success: true, error: []};
                }
            }
        }

        return {success: false, error: ['There is no user registered with these credentials.']}
    };
