...
var Query = (function () {
    ...
    function Query(query, database) {
        this.cache = require('jdb-cache')(this);
        this.queryParser = require('jdb-queryparser');
    ...
    Query.prototype._prepare = function () {
        ...
        this.queryExecuted = false;
        return require('jdb-preparedquerystatement')(instance.getQueryString(), instance);
    };
    ...
    Query.prototype._execute = function () {
        if (!this.queryIsExecuted()) {
            if (null === this.database || null === this.parsedQuery) {
                throw new Error("Query Error: Can't execute the query. No database/table selected or internal error.");
            }

            this.setTable(this.parsedQuery.table);

            var Util = require('jdb-util');
            ...
            if (!Util.existsSync(table_path)) {
                throw new Error("Query Error: Can't execute the query. The table \"" + this.table + "\" doesn't exists in database \"" + this.database + "\" or file access denied.");
            }
    ...
    Query.prototype.async._execute = function (callback) {
        if (!instance.queryIsExecuted()) {
            if (null === instance.database || null === instance.parsedQuery) {
                callback(new Error("Query Error: Can't execute the query. No database/table selected or internal error."), null);
            }
            ...
            Util.exists(table_path, function (exists) {
                if (!exists) {
                    callback(new Error("Query Error: Can't execute the query. The table \"" + instance.table + "\" doesn't exists in database \"" + instance.database + "\" or file access denied."), null);
                } else {
    ...
    Query.prototype.setTable = function (table) {
        if (null === this.database.getDatabase()) {
            throw new Error("Query Error: Can't use the table \"" + table + "\", there is no database selected.");
        }
        var Util = require('jdb-util');
        var path = Util._getTablePath(this.database.getServer(), this.database.getDatabase(), table);
        if (!(Util.existsSync(path))) {
            throw new Error("Query Error: Can't use the table \"" + table + "\", the table doesn't exist in the database.");
        }
        this.table = table;
        return this;
    };
    ...
    Query.prototype._select = function (data, callback) {
        ...
        if (null === callback || !(typeof callback === 'function')) {
            return require('jdb-queryresult')(this.queryResults, this);
        } else {
            callback(null, require('jdb-queryresult')(this.queryResults, this));
        }
    };
    ...
    Query.prototype._count = function (data, callback) {
        ...
        if (null === callback || !(typeof callback === 'function')) {
            return require('jdb-queryresult')(result, this);
        } else {
            callback(null, require('jdb-queryresult')(result, this));
        }
    };
