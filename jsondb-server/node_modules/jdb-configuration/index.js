...
var Configuration = (function () {
    ...
    Configuration.prototype.getConfig = function (filename) {
        ...
        if (this._exists(filename)) {
            return JSON.parse(_f.readFileSync(_p.normalize(_p.dirname(__filename) + '/config/' + filename + '.json')));
        } else {
    ...
    Configuration.prototype._writeConfig = function (filename, config) {
        var _f = require('fs');
        var _p = require('path');
        var path = _p.normalize(_p.dirname(__filename) + '/config/' + filename + '.json');
        if (!this._exists(filename)) {
    ...
    Configuration.prototype._exists = function (filename) {
        var Util = require('jdb-util');
        var _p = require('path');
        return Util.existsSync(_p.normalize(_p.dirname(__filename) + '/config/' + filename + '.json'));
    };
